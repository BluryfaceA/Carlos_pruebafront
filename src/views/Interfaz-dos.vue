<template>
    <div class="min-h-full">
      ...
      <header class="bg-white shadow">
        <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
          <h1 class="text-3xl font-bold tracking-tight text-gray-900 mb-6">Dashboard / Users</h1>
            ...
            <table class="min-w-full divide-y divide-gray-300 border border-gray-300 rounded-lg shadow-lg">
              ...
              <tbody class="bg-white text-gray-800 divide-y divide-gray-200">
                <tr v-for="datos in MostrarFiltrados" :key="datos.id" class="hover:bg-gray-100 transition duration-200">
                  <td class="px-6 py-4 text-sm font-medium">{{ datos.name }}</td>
                  <td class="px-6 py-4 text-sm">{{ datos.username }}</td>
                  <td class="px-6 py-4 text-sm">{{ datos.email }}</td>

                  <td class="px-6 py-4 text-sm"> 
                     
              <div class="relative inline-block text-justify">
                        ...
                        <div class="py-1">
                          <MenuItem v-slot="{ active }">
                            <a href="#" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-sm']">
                              <strong>Teléfono:</strong> {{ datos.phone }}
                            </a>
                          ...
                          <MenuItem v-slot="{ active }">
                            <a href="#" :class="[active ? 'bg-gray-100 text-gray-900' : 'text-gray-700', 'block px-4 py-2 text-sm']">
                              <strong>Sitio Web:</strong>   {{ datos.website}}  
                            </a> 
                          </MenuItem>
  ...
  const navigation = [
    { name: 'Dashboard / Users', href: '/interfaz-dos', current: true },
    { name: 'Ir Dasboard / Posts', href: '/interfaz-uno', current: false },
  ]
